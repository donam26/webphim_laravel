                        ...
                        <ul id="menu-menu_1" class="nav navbar-nav navbar-left">
                            ...
                            @foreach ($category as $cate)
                                <li class="mega"><a title="{{ $cate->title }}"
                                        href="{{ route('category', Illuminate\Support\Str::of($cate->title)->slug('-')) }}">{{ $cate->title }}</a>
                                </li>
                            @endforeach
                                ...
                                <ul role="menu" class=" dropdown-menu">
                                    @foreach ($genre as $gen)
                                        <li><a title="{{ $gen->title }}"
                                                href="{{ route('genre', Illuminate\Support\Str::of($gen->title)->slug('-')) }}">{{ $gen->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
                                ...
                                <ul role="menu" class=" dropdown-menu">
                                    @foreach ($country as $coun)
                                        <li><a title="{{ $coun->title }}"
                                                href="{{ route('country', Illuminate\Support\Str::of($coun->title)->slug('-')) }}">{{ $coun->title }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#result').on('click', 'li', function() {
                var click = $(this).text().split('|');
                $('#timkiem').val($.trim(click[0]));
                $('#result').html('');
                $('#result').css('display', 'none');
            });
        });
